...
/decl/surgery_step/generic/cut_with_laser/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	affected.createwound(CUT, affected.min_broken_damage*0.5, 1)
...
/decl/surgery_step/generic/managed/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	affected.createwound(CUT, affected.min_broken_damage*0.5, 1) // incision
...
/decl/surgery_step/generic/amputate/proc/is_clean(mob/user, obj/item/tool, mob/target)
	. = (user.a_intent != I_HELP) ? FALSE : (can_operate(target) >= OPERATE_OKAY && istype(tool, /obj/item/circular_saw))

/decl/surgery_step/generic/amputate/get_speed_modifier(mob/user, mob/target, obj/item/tool)
	. = ..()

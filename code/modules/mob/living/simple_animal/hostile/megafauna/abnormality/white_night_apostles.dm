...
/obj/item/natural_weapon/apostle_scythe
	...
	hitsound = 'sound/simple_mob/abnormality/white_night/scythe.ogg'
	force = 36
	sharp = TRUE
	...
	for(var/turf/T in view(scythe_range, src))
		...
		for(var/mob/living/L in T)
			...
			if((L.stat == DEAD) || (L.getBruteLoss() >= L.maxHealth * 3))
				...
				gibbed = TRUE
	playsound(get_turf(src), (gibbed ? 'sound/simple_mob/abnormality/white_night/scythe_gib.ogg' : 'sound/simple_mob/abnormality/white_night/scythe_spell.ogg'), (gibbed ? 100 : 75), FALSE, (gibbed ? 12 : 5))
	SLEEP_CHECK_DEATH(5)
	can_act = TRUE
...
/obj/item/natural_weapon/apostle_spear
	...
	hitsound = 'sound/simple_mob/abnormality/white_night/spear.ogg'
	force = 25
	sharp = TRUE
	edge = 1
	melee_accuracy_bonus = 200
...
/mob/living/simple_animal/hostile/apostle/spear/PerformAbility(atom/target)
	...
	playsound(get_turf(src), 'sound/simple_mob/abnormality/white_night/spear_charge.ogg', 75, 0, 5)
	SLEEP_CHECK_DEATH(22)
	been_hit = list()
	playsound(get_turf(src), 'sound/simple_mob/abnormality/white_night/spear_dash.ogg', 100, 0, 20)

/mob/living/simple_animal/hostile/megafauna/white_night
	...
	deathsound = 'sound/simple_mob/abnormality/white_night/apostle_death.ogg'

	health = 15000
	maxHealth = 15000

	movement_cooldown = 8
	movement_sound = 'sound/simple_mob/abnormality/white_night/apostle_movement.ogg'
...
/mob/living/simple_animal/hostile/megafauna/white_night/Stat()
	. = ..()
...
/mob/living/simple_animal/hostile/megafauna/white_night/proc/InitiateConversion(mob/living/carbon/human/H)
	...
	playsound(src, 'sound/simple_mob/abnormality/white_night/apostle_convert.ogg', 50, FALSE, 4)
...
/mob/living/simple_animal/hostile/megafauna/white_night/proc/HumanConversion(mob/living/carbon/human/H)
	...
	for(var/mob/M in GLOB.player_list)
		if((M.z in GetConnectedZlevels(z)) && M.client)
			...
			M.playsound_local(get_turf(M), 'sound/simple_mob/abnormality/white_night/apostle_bell.ogg', 75, FALSE)
...
/mob/living/simple_animal/hostile/megafauna/white_night/proc/HolyRevival()
	...
	playsound(src, 'sound/simple_mob/abnormality/white_night/apostle_spell.ogg', 100, TRUE, 7)
		...
		for(var/turf/T in spiral_range_turfs(i, target_c) - spiral_range_turfs(i-1, target_c))
			...
			for(var/mob/living/L in T)
				if(L.faction != faction)
					if(L.stat == DEAD)
						continue
					playsound(L, 'sound/simple_mob/abnormality/white_night/ark_damage.ogg', 50, TRUE, -1)
...
/mob/living/simple_animal/hostile/megafauna/white_night/proc/ChargedField()
	...
	playsound(src, 'sound/simple_mob/abnormality/white_night/apostle_charge.ogg', 75, FALSE, 7)
	for(var/i = 1 to 2)
	...
	// Scare everyone
	playsound(src, 'sound/simple_mob/abnormality/white_night/apostle_fire.ogg', 100, FALSE, 14)
	...
	for(var/i = 1 to charged_field_range)
		for(var/turf/T in spiral_range_turfs(i, target_c) - spiral_range_turfs(i-1, target_c))
			...
			for(var/mob/living/L in T)
				...
				if(L.stat == DEAD)
					continue
				playsound(L, 'sound/simple_mob/abnormality/white_night/ark_damage.ogg', 50, TRUE)
...
/mob/living/simple_animal/hostile/megafauna/white_night/proc/Rapture()
	...
	for(var/mob/M in GLOB.player_list)
		if(M.client && !isnewplayer(M)) // Send it to every player currently active(outside of lobby), not just everyone on Z-level
			M.playsound_local(get_turf(M), 'sound/simple_mob/abnormality/white_night/rapture.ogg', 50)
	...
	for(var/i = 1 to apostles.len)
		...
		for(var/mob/M in GLOB.player_list)
			if((M.z in GetConnectedZlevels(z)) && M.client)
				...
				M.playsound_local(get_turf(M), 'sound/simple_mob/abnormality/white_night/apostle_bell.ogg', 100)
	...
	for(var/mob/M in GLOB.player_list)
		if(M.z == z && M.client)
			M.playsound_local(get_turf(M), 'sound/simple_mob/abnormality/white_night/rapture2.ogg', 50)

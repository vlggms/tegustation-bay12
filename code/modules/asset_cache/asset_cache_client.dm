...
/client/proc/asset_cache_update_json()
	...
	show_browser(src, json_encode(sent_assets), "file=asset_data.json&display=0")
...
/client/proc/browse_queue_flush(timeout = 50)
	...
	show_browser(src, {"<script>window.location.href=\"?asset_cache_confirm_arrival=[job]\"</script>"}, "window=asset_cache_browser&file=asset_cache_send_verify.htm")

SUBSYSTEM_DEF(dbcore)
	...
	///! NOTE: SStime_track is not in yet, but these are essential to other functions.
...
/datum/controller/subsystem/dbcore/proc/create_active_query(datum/db_query/query)
	...
	//if(IsAdminAdvancedProcCall()) //Disabled because not present in codebase, currently OOS for implementation
	//	return FALSE
	run_query(query)
	queries_active_num++
	queries_active += query
...
/datum/controller/subsystem/dbcore/Shutdown()
	...
	if(SSdbcore.Connect())
		...
		var/datum/db_query/query_round_shutdown = SSdbcore.NewQuery(
			...
			list("end_state" = SSticker.end_game_state, "round_id" = game_id)
		)
...
/datum/controller/subsystem/dbcore/proc/Connect()
	...
	//var/timeout = max(CONFIG_GET(number/async_query_timeout), CONFIG_GET(number/blocking_query_timeout)) // Commented out until it matters
	var/timeout = max(10, 5)
	//var/thread_limit = CONFIG_GET(number/bsql_thread_limit) // See above
	var/thread_limit = 50

	//max_concurrent_queries = CONFIG_GET(number/max_concurrent_queries) // See above
	max_concurrent_queries = 25 // TODO: Make the above matter
...
/datum/controller/subsystem/dbcore/proc/SetRoundEnd()
	...
	var/datum/db_query/query_round_end = SSdbcore.NewQuery(
		...
		list("game_mode_result" = SSticker.master_mode, "station_name" = station_name(), "round_id" = game_id)
	)
...
// /datum/db_query/CanProcCall(proc_name) // Implementation OOS
// 	//fuck off kevinz
// 	return FALSE
